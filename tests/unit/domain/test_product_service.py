...
@pytest.mark.asyncio
async def test_get_product_caches_result(service, mock_repo, mock_cache):
    product = Product(id=1, name="Test", sku="SKU-001", price=Decimal("10.00"), stock=5)
    mock_repo.get_by_id.return_value = product  # Simulate cache hit
...
@pytest.mark.asyncio
async def test_get_products_returns_paginated(service, mock_repo, mock_cache):
    products = [
